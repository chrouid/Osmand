...
class ShowLocationHelper(private val app: TelegramApplication) {
	...
	fun addOrUpdateLocationOnMap(message: TdApi.Message, update: Boolean = false) {
		execOsmandApi {
			...
			if (chatTitle != null && (content is TdApi.MessageLocation || (content is OsmandLocationUtils.MessageUserTextLocation && content.isValid()))) {
				var userName = ""
				...
				val aLatLon = when (content) {
					...
					is OsmandLocationUtils.MessageUserTextLocation -> ALatLon(content.lat, content.lon)
					else -> null
				}
				if (aLatLon != null) {
					if (update) {
