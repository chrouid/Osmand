...
import net.osmand.telegram.helpers.MessagesDbHelper
...
object OsmandLocationUtils {
	...
	private val log = PlatformUtil.getLog(OsmandLocationUtils::class.java)
	...
	private const val SHARING_LINK =
	...
	fun parseMessage(
		...
		status: Int
	): MessagesDbHelper.LocationMessage? {
		var locationMessage: MessagesDbHelper.LocationMessage? = null
		...
		if (parsedMessageContent != null) {
			locationMessage = MessagesDbHelper.LocationMessage(
				message.senderUserId,
				message.chatId,
				parsedMessageContent.lat,
				parsedMessageContent.lon,
				parsedMessageContent.altitude,
				parsedMessageContent.speed,
				parsedMessageContent.hdop,
				parsedMessageContent.lastUpdated * 1000L,
				messageType,
				status,
				message.id
			)
	...
	fun formatLocation(sig: MessagesDbHelper.LocationMessage): String {
	...
	fun getTextMessageContent(updateId: Int, location: Location): TdApi.InputMessageText {
		...
		entities.add(
			TdApi.TextEntity(
				builder.length, locationMessage.length,
		...
		if (location.accuracy != 0.0f && location.speed == 0.0f) {
			entities.add(
				TdApi.TextEntity(
					builder.lastIndex,
					HDOP_PREFIX.length,
		...
		return TdApi.InputMessageText(
			TdApi.FormattedText(textMessage, entities.toTypedArray()),
			true,
			true
		)
	}

	fun getTextMessageContent(
		updateId: Int,
		location: MessagesDbHelper.LocationMessage
	...
	fun parseOsmAndBotLocationContent(
		oldContent: MessageOsmAndBotLocation,
	...
	fun parseTextLocation(
		text: TdApi.FormattedText,
		botLocation: Boolean = true
	): MessageLocation {
	...
	fun convertLocationMessagesToGpxFiles(
		items: List<MessagesDbHelper.LocationMessage>,
		newGpxPerChat: Boolean = true
	): List<GPXUtilities.GPXFile> {
	...
	fun saveGpx(
		...
		listener: SaveGpxListener,
		dir: File,
		gpxFile: GPXUtilities.GPXFile
	) {
		if (!gpxFile.isEmpty) {
		...
		override fun doInBackground(vararg params: Void): List<String> {
			...
			if (dir.parentFile.canWrite()) {
				if (dir.exists()) {
					...
					// save file
					var fout = File(dir, userId.toString() + ".gpx") //$NON-NLS-1$
					if (!gpxFile.isEmpty) {
						...
						fileName = if (user != null) {
							(TelegramUiHelper.getUserName(user)
									+ "_" + SimpleDateFormat(
								"yyyy-MM-dd_HH-mm_EEE",
								Locale.US
							).format(Date(pt.time))) //$NON-NLS-1$
						} else {
							userId.toString() + "_" +
									SimpleDateFormat("yyyy-MM-dd_HH-mm_EEE", Locale.US).format(
										Date(
											pt.time
										)
									) //$NON-NLS-1$
						}
						fout = File(dir, "$fileName.gpx") //$NON-NLS-1$
						var ind = 1
						while (fout.exists()) {
							fout = File(
								dir,
